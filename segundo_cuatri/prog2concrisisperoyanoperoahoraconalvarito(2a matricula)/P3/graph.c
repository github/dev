...
Status graph_breathSearch(Graph *g, long from_id, long to_id)
{
    ...
    Vertex *v1 = NULL, *v2 = NULL, *vaux = NULL;
    v1 = getvertexbyID(g, from_id);
    v2 = getvertexbyID(g, to_id);
    int i;
    for (int i = 0; i < graph_getNumberOfVertices(g); i++)
    {
        vaux = getvertexbyind(g, i);
    ...
    while (!queue_isEmpty(q) && vertex_getState(v2) != BLACK)
    {
        Vertex *v = queue_pop(q);

        if (vertex_cmp(v, v2) == 0)
        {
        ...
        else
        {
            if (vertex_getState(v) == WHITE)
            {
                ...
                long *conections = graph_getConnectionsFromId(g, vertex_getId(v));

                for (i = 0; i < graph_getNumberOfConnectionsFromId(g, vertex_getId(v)); i++)
                {
                    Vertex *v3 = getvertexbyID(g, conections[i]);

                    if (vertex_getState(v3) == WHITE)
                    {
                        if (!queue_push(q, v3))
                        {
                            return ERROR;
